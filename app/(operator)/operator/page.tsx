...
import { allStockRows, summaryFor } from "@/lib/repo/inventory";
import {
  indexById,
 ...
 */
export async function operatorCatalogue(siteId: string): Promise<OperatorProduct[]> {
  const [products, stockRows, locationById, warehouseById] = await Promise.all([
    allProducts(),
    allStockRows(),
  ...
  return Promise.all(
    products
      ...
      .map(async (product) => {
        const rows = stockRows.filter((s) => s.productId === product.id);
        const here = rows.filter((s) => s.warehouseId === siteId);
        const summary = await summaryFor(product.id);
...
export default async function OperatorLookupPage() {
  ...
  return <LookupClient products={await operatorCatalogue(site.id)} siteCode={site.code} />;
}
